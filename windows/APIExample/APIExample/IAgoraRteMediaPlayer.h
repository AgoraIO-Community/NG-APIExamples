...
class IAgoraRteMediaPlayerObserver {
 public:
  ...
  virtual void OnPlayerEvent(const RteFileInfo& current_file_info,
                             MEDIA_PLAYER_EVENT event) = 0;

  /**
   * Occurs when all media files playback is completed.
   *
   * @param err_code The error code is emulate with
   * media::base::MEDIA_PLAYER_ERROR
   */
   ...
   *
   * @param current_file_info The played file information.
   * @param playCachedBuffer The updated palyer buffer.
   */
  virtual void OnPlayerBufferUpdated(const RteFileInfo& current_file_info,
                                     int64_t playCachedBuffer) = 0;
 ...
 protected:
  ~IAgoraRteMediaPlayerObserver() = default;
};
...
class IAgoraRteMediaPlayer {
 public:
   ...
   * - < 0: Failure.
   */
  virtual int ChangePlaybackSpeed(MEDIA_PLAYER_PLAYBACK_SPEED speed) = 0;
