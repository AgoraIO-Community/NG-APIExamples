...
void CAPIExampleDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_CHECK_COMPATIBLE, m_chkCompatible);
}
...
BEGIN_MESSAGE_MAP(CAPIExampleDlg, CDialogEx)
	...
	ON_WM_CLOSE()
	ON_BN_CLICKED(IDC_CHECK_COMPATIBLE, &CAPIExampleDlg::OnClickedCheckCompatible)
END_MESSAGE_MAP()
...
void CAPIExampleDlg::InitSceneDialog()
{
   ...
   //custom audio video capture
   m_pCaputreVideoDlg = std::make_unique<CAgoraAVCaptureDlg>(&m_staMainArea);
   m_pCaputreVideoDlg->Create(CAgoraAVCaptureDlg::IDD);
   m_pCaputreVideoDlg->MoveWindow(&rcWnd);
   m_vecAdvanced.push_back(advancedCustomAVCapture);
...
{
	m_chkCompatible.EnableWindow(FALSE);
	...
	else if (selectedText.Compare(advancedCustomAVCapture) == 0) {
...
void CAPIExampleDlg::ReleaseScene(CTreeCtrl& treeScene, HTREEITEM& hSelectItem)
{
	...
	else if (str.Compare(advancedCustomAVCapture) == 0) {
...
void CAPIExampleDlg::OnClickedCheckCompatible()
{
	// TODO: Add your control notification handler code here

	bCompatible = m_chkCompatible.GetCheck();
}
