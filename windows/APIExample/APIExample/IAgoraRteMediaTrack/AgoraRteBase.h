...
using MEDIA_PLAYER_PLAYBACK_SPEED = media::base::MEDIA_PLAYER_PLAYBACK_SPEED;
using VIDEO_MIRROR_MODE_TYPE = rtc::VIDEO_MIRROR_MODE_TYPE;
using RENDER_MODE_TYPE = media::base::RENDER_MODE_TYPE;
using AUDIO_REVERB_PRESET = rtc::AUDIO_REVERB_PRESET;
using VOICE_CHANGER_PRESET = rtc::VOICE_CHANGER_PRESET;
...
using VideoCanvas = rtc::VideoCanvas;
...
struct StreamInfo {
  ...
  std::string user_id;
  /**
   * Reserved, store the VOS uid, please ignore it
   */
  uint32_t reversed_id;
};
...
//
struct StreamOption {
  StreamOption(StreamType input_type) : type(input_type) {}

  virtual ~StreamOption() = default;

  virtual void Assign(const StreamOption&) = 0;
...
struct RtcStreamOptions : public StreamOption {
   ...
   * @param option
   */
  void Assign(const StreamOption& option) override {
    if (option.type == StreamType::kRtcStream) {
      token = static_cast<const RtcStreamOptions&>(option).token;
    }
  }

  /**
   * The token for joining scene
   * // TODO (yejun): how to generate token
   */
  std::string token;
};
...
struct DirectCdnStreamOptions : public StreamOption {
  ...
  void Assign(const StreamOption& option) override {
    if (option.type == StreamType::kCdnStream) {
      url = static_cast<const DirectCdnStreamOptions&>(option).url;
    }
  }
...
struct SceneConfig {
  ...
  /**
   * Flag to tell scene to disable audio recording and playing, this flag is
   * only for pushing customized data (e.g. data from MediaPlayerKit) to
   * network.
   */
 ...
 */
enum CDNBYPASS_STREAM_PUBLISH_STATE {
  /**
   ...
   */
  CDNBYPASS_STREAM_PUBLISH_STATE_IDLE = 0,
  /**
   ...
   * `addPublishStreamUrl`.
   */
  CDNBYPASS_STREAM_PUBLISH_STATE_RECOVERING = 3,
  /**
   ...
   */
  CDNBYPASS_STREAM_PUBLISH_STATE_FAILURE = 4,
};

/**
 * Error codes of the CDN(RTMP) bypass streaming.
 */
enum CDNBYPASS_STREAM_PUBLISH_ERROR {
  /**
   * -1: The RTMP streaming fails.
   */
  CDNBYPASS_STREAM_PUBLISH_ERROR_FAILED = -1,
  /**
   * 0: The RTMP streaming publishes successfully.
   */
  CDNBYPASS_STREAM_PUBLISH_ERROR_OK = 0,
  /**
   ...
   */
  CDNBYPASS_STREAM_PUBLISH_ERROR_CDN_ERROR = 11,
  /**
   * 12: Resources are occupied and cannot be reused.
   */
  CDNBYPASS_STREAM_PUBLISH_ERROR_ALREADY_IN_USE = 12,
};
...
struct RemoteStreamStats {
  RteRemoteAudioStats audio_stats;
  RteRemoteVideoStats video_stats;
};
