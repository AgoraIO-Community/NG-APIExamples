...
class AgoraRteUtils {
 public:
  ...
  static void NotifyEventHandlers(
      std::mutex& mutex, std::vector<std::weak_ptr<EventHandlerT>>& handlers,
  ...
  static void NotifyEventHandlers(
      ...
      std::vector<std::weak_ptr<EventHandlerT>>& handlers,
      std::function<void(const std::shared_ptr<EventHandlerT>&)> task) {
    std::vector<std::shared_ptr<EventHandlerT>> shared_handles;
...
class AgoraRteLocalStream : public std::enable_shared_from_this<AgoraRteLocalStream>,
 ...
 public:
  ...
  virtual uint32_t GetLocalUid() = 0;
...
class AgoraRteCdnLocalStream : public AgoraRteLocalStream,
 ...
 public:
  ...
  virtual uint32_t GetLocalUid() override;
...
class AgoraRteRtcStreamBase {
 public:
  ...
  uint32_t GetLocalUid() { return local_uid_; }
...
class AgoraRteRtcMajorStream final : public AgoraRteRtcStreamBase,
 ...
 public:
  ...
  std::shared_ptr<AgoraRteRtcRemoteStream>
  FindRemoteStream(const agora_refptr<rtc::IRemoteVideoTrack>& videoTrack);

  std::shared_ptr<AgoraRteRtcRemoteStream>
  FindRemoteStream(const agora_refptr<rtc::IRemoteAudioTrack>& audioTrack);

  int PushUserInfo(const UserInfo& info, InstanceState state) override {
    return ERR_OK;
  }

  int PushStreamInfo(const StreamInfo& info,
...
class AgoraRteRtcRemoteStream final : public AgoraRteRemoteStream {
 public:
  ...
  std::shared_ptr<rtc::IRemoteAudioTrack> GetAudioTrack() const { return audio_track_;}

  std::shared_ptr<rtc::IRemoteVideoTrack> GetVideoTrack() const { return video_track_;}
...
namespace agora {
namespace rte {
class AgoraRteRtCompatiblecStreamObserver;
class AgoraRteRtcCompatibleUserObserver;
class AgoraRteRtCompatiblecAudioFrameObserver;
...
class AgoraRteRtcCompatibleMajorStream final : public AgoraRteRtcStreamBase,
 ...
 private:
  ...
  std::shared_ptr<RtcCallbackWrapper<AgoraRteRtCompatiblecStreamObserver>>
      rtc_major_stream_observer_;
...
class AgoraRteRtcCompatibleLocalStream final : public AgoraRteLocalStream {
 public:
  ...
  virtual uint32_t GetLocalUid() override {
    return major_stream_->GetLocalUid();
  }
  virtual int UpdateOption(const StreamOption& option) override {
...
class AgoraRteRtCompatiblecStreamObserver : public rtc::IRtcConnectionObserver {
 public:
  AgoraRteRtCompatiblecStreamObserver(
      const std::shared_ptr<AgoraRteScene>& scene,
...
class AgoraRteRtCompatiblecAudioFrameObserver
 ...
 public:
  AgoraRteRtCompatiblecAudioFrameObserver(
      const std::shared_ptr<AgoraRteScene>& scene,
  ...
  virtual bool onPlaybackAudioFrameBeforeMixing(
      unsigned int uid, AudioFrame& audioFrame) override;
...
inline int AgoraRteCameraVideoTrack::SetCameraFaceDetection(bool enable) {
  return camera_capturer_->setCameraFaceDetection(enable);
}
#elif RTE_DESKTOP
...
inline uint32_t AgoraRteCdnLocalStream::GetLocalUid() { return 0; };
...
#define ERR_CODE(err) (-1 * static_cast<int>(err))
...
inline bool AgoraRtePlayList::CurrentIsFirstFile() {
  ...
  if ((file_list_.size() <= 0) || (current_file_ == nullptr)) {
    return false;
  }
...
inline int AgoraRteRtcCompatibleMajorStream::RegisterObservers() {
  ...
  if (scene) {
    rtc_major_stream_observer_ =
        RtcCallbackWrapper<AgoraRteRtCompatiblecStreamObserver>::Create(
    ...
    result = internal_rtc_connection_->registerObserver(
        rtc_major_stream_observer_.get(), [](rtc::IRtcConnectionObserver* obs) {
          auto wrapper = static_cast<agora::rte::RtcCallbackWrapper<
              AgoraRteRtCompatiblecStreamObserver>*>(obs);
          wrapper->DeleteMe();
        });
...
inline int AgoraRteRtcCompatibleMajorStream::RemoveBypassCdnUrl(
    const std::string& target_cdn_url) {
  CreateRtmpService();
  return rtmp_service_->removePublishStreamUrl(target_cdn_url.c_str());
}
...
inline AgoraRteRtCompatiblecAudioFrameObserver::
    AgoraRteRtCompatiblecAudioFrameObserver(
        const std::shared_ptr<AgoraRteScene>& scene,
        const std::shared_ptr<AgoraRteRtcStreamBase>& stream) {
  scene_ = scene;
...
inline AgoraRteRtCompatiblecStreamObserver::
    ~AgoraRteRtCompatiblecStreamObserver() {
...
inline void AgoraRteRtCompatiblecStreamObserver::onConnected(
    const rtc::TConnectionInfo& connectionInfo,
...
inline void AgoraRteRtCompatiblecStreamObserver::onDisconnected(
    const rtc::TConnectionInfo& connectionInfo,
    rtc::CONNECTION_CHANGED_REASON_TYPE reason) {
...
inline void AgoraRteRtCompatiblecStreamObserver::onConnecting(
    const rtc::TConnectionInfo& connectionInfo,
    rtc::CONNECTION_CHANGED_REASON_TYPE reason) {
...
inline void AgoraRteRtCompatiblecStreamObserver::onReconnecting(
    const rtc::TConnectionInfo& connectionInfo,
    rtc::CONNECTION_CHANGED_REASON_TYPE reason) {
...
inline void AgoraRteRtCompatiblecStreamObserver::onReconnected(
    const rtc::TConnectionInfo& connectionInfo,
    rtc::CONNECTION_CHANGED_REASON_TYPE reason) {
...
inline void AgoraRteRtCompatiblecStreamObserver::onConnectionLost(
    const rtc::TConnectionInfo& connectionInfo) {
...
inline void AgoraRteRtCompatiblecStreamObserver::onUserJoined(
    user_id_t userId) {
...
inline void AgoraRteRtCompatiblecStreamObserver::onUserLeft(
...
inline void AgoraRteRtCompatiblecStreamObserver::onTokenPrivilegeWillExpire(
    const char* token) {
  auto local_stream = stream_.lock();
  auto scene = scene_.lock();
...
inline void
AgoraRteRtCompatiblecStreamObserver::onTokenPrivilegeDidExpire() {
  auto local_stream = stream_.lock();
  auto scene = scene_.lock();
...
inline void AgoraRteRtCompatiblecStreamObserver::onLastmileQuality(
    const rtc::QUALITY_TYPE quality) {}

inline void AgoraRteRtCompatiblecStreamObserver::onLastmileProbeResult(
    const rtc::LastmileProbeResult& result) {}

inline void AgoraRteRtCompatiblecStreamObserver::onConnectionFailure(
    const rtc::TConnectionInfo& connectionInfo,
    rtc::CONNECTION_CHANGED_REASON_TYPE reason) {
  auto stream = stream_.lock();
  auto scene = scene_.lock();
...
inline void AgoraRteRtCompatiblecStreamObserver::onTransportStats(
    const rtc::RtcStats& stats) {}

inline void
AgoraRteRtCompatiblecStreamObserver::onChannelMediaRelayStateChanged(int state,
                                                                     int code) {
}
...
inline void AgoraRteRtcCompatibleUserObserver::onPublishStateChangedCommon(
  ...
  if (scene && stream) {
    ...
    if (notify_callback) {
      ...
      info.reversed_id = stream->GetLocalUid();
      ...
      if (state == PublishState::kPublished) {
        ...
        scene->OnLocalStreamStateChanged(
            info, type, StreamMediaState::kStreaming, StreamMediaState::kIdle,
...
inline void AgoraRteRtcCompatibleUserObserver::onAudioPublishStateChanged(
    const char* channel, rtc::STREAM_PUBLISH_STATE oldState,
    rtc::STREAM_PUBLISH_STATE newState, int elapseSinceLastState) {
  onPublishStateChangedCommon(channel, oldState, newState, elapseSinceLastState,
                              MediaType::kAudio);
}
...
inline bool AgoraRteRtCompatiblecAudioFrameObserver::onRecordAudioFrame(
    AudioFrame& audioFrame) {
  auto scene = scene_.lock();
  if (scene) {
...
inline bool AgoraRteRtCompatiblecAudioFrameObserver::onPlaybackAudioFrame(
    AudioFrame& audioFrame) {
  auto scene = scene_.lock();
  if (scene) {
...
inline bool AgoraRteRtCompatiblecAudioFrameObserver::onMixedAudioFrame(
    AudioFrame& audioFrame) {
  auto scene = scene_.lock();
  if (scene) {
...
inline bool
AgoraRteRtCompatiblecAudioFrameObserver::onPlaybackAudioFrameBeforeMixing(
    unsigned int uid, AudioFrame& audioFrame) {
  auto scene = scene_.lock();
  if (scene) {
    // TODO change the uid to right string user id
    //
    scene->OnPlaybackAudioFrameBeforeMixing("", audioFrame);
  }
  return true;
}
...
inline uint32_t AgoraRteRtcLocalStream::GetLocalUid() {
  return AgoraRteRtcStreamBase::GetLocalUid();
};
...
inline int AgoraRteRtcRemoteStream::AdjustRemoteVolume(int volume) {
  ...
  return rtc_connection->getLocalUser()->adjustUserPlaybackSignalVolume(
...
void Convert::LocalAudioStats(const rtc::LocalAudioStats& stats,
                              RteLocalAudioStats& dest_stats) {
...
void Convert::LocalVideoStats(const rtc::LocalVideoTrackStats& stats,
                              RteLocalVideoStats& dest_stats) {
  ...
  dest_stats.encodedFrameCount = stats.frames_encoded;
}

void Convert::RemoteAudioStats(const rtc::RemoteAudioTrackStats& stats,
                               RteRemoteAudioStats& dest_stats) {
  dest_stats.quality = stats.quality;
  dest_stats.networkTransportDelay = stats.network_transport_delay;
  dest_stats.jitterBufferDelay = stats.jitter_buffer_delay;
  ...
  dest_stats.mosValue = stats.mos_value;
}

void Convert::RemoteVideoStats(const rtc::RemoteVideoTrackStats& stats,
                               RteRemoteVideoStats dest_stats) {
...
void AgoraRteRtcLocalStreamObserver::onConnected(
  const rtc::TConnectionInfo& connectionInfo,
  rtc::CONNECTION_CHANGED_REASON_TYPE reason) {
  auto stream = stream_.lock();
  if (stream) {
...
void AgoraRteRtcLocalStreamObserver::onConnecting(
  const rtc::TConnectionInfo& connectionInfo,
  rtc::CONNECTION_CHANGED_REASON_TYPE reason) {
  auto stream = stream_.lock();
  if (stream) {
...
void AgoraRteRtcLocalStreamObserver::onReconnecting(
  const rtc::TConnectionInfo& connectionInfo,
  rtc::CONNECTION_CHANGED_REASON_TYPE reason) {
  auto stream = stream_.lock();
  if (stream) {
...
void AgoraRteRtcLocalStreamObserver::onReconnected(
  const rtc::TConnectionInfo& connectionInfo,
  rtc::CONNECTION_CHANGED_REASON_TYPE reason) {
  auto stream = stream_.lock();
  if (stream) {
...
inline void AgoraRteRtcLocalStreamUserObserver::onLocalAudioTrackStatistics(
  ...
  if (stream && scene) {
    ...
    Convert::LocalAudioStats(stats, audio_stats);
...
AgoraRteRtcMajorStreamUserObserver::onRemoteAudioTrackStatistics(
  ...
  if (scene && stream) {
    RteRemoteAudioStats dest_stats;
    Convert::RemoteAudioStats(stats, dest_stats);
...
inline void AgoraRteRtcLocalStreamUserObserver::onLocalVideoTrackStatistics(
  ...
  if (stream && scene) {
    ...
    Convert::LocalVideoStats(stats, video_stats);
...
AgoraRteRtcMajorStreamUserObserver::onRemoteVideoTrackStatistics(
  ...
  if (stream && scene) {
    ...
    if (remote_stream) {
      RteRemoteVideoStats dest_stats = {};
      Convert::RemoteVideoStats(stats, dest_stats);
...
inline void AgoraRteRtcLocalStreamUserObserver::onPublishStateChangedCommon(
  const char* channel, rtc::STREAM_PUBLISH_STATE oldState,
  rtc::STREAM_PUBLISH_STATE newState, int elapseSinceLastState,
  MediaType type) {
  assert(type == MediaType::kAudio || type == MediaType::kVideo);
...
inline void AgoraRteRtcRemoteVideoObserver::onFrame(
  user_id_t uid, rtc::conn_id_t connectionId,
  const media::base::VideoFrame* frame) {
  auto scene = scene_.lock();
  if (scene) {
    RteRtcStreamInfo info;
    std::string rtc_stream_id(uid);
...
inline void AgoraRteScene::OnRtcStats(const std::string& stream_id,
  ...
  for (auto& rtc_stats_vec : rtc_stats_map_) {
    for (auto& rtc_stats : rtc_stats_vec.second) {
      scene_stats.txBytes += rtc_stats.txBytes;
      scene_stats.rxBytes += rtc_stats.rxBytes;
      scene_stats.txAudioBytes += rtc_stats.txAudioBytes;
      scene_stats.txVideoBytes += rtc_stats.txVideoBytes;
      scene_stats.rxAudioBytes += rtc_stats.rxAudioBytes;
      scene_stats.rxVideoBytes += rtc_stats.rxVideoBytes;
      scene_stats.txKBitRate += rtc_stats.txKBitRate;
      scene_stats.rxKBitRate += rtc_stats.rxKBitRate;
      scene_stats.txAudioKBitRate += rtc_stats.txAudioKBitRate;
      scene_stats.rxAudioKBitRate += rtc_stats.rxAudioKBitRate;
      scene_stats.txVideoKBitRate += rtc_stats.txVideoKBitRate;
      scene_stats.rxVideoKBitRate += rtc_stats.rxVideoKBitRate;
    }
  }
...
void AgoraRteScene::OnLocalStreamStats(const std::string& stream_id,const LocalStreamStats& stats) {
  AgoraRteUtils::NotifyEventHandlers<IAgoraRteSceneEventHandler>(
    callback_lock_, event_handlers_,
    [&](const auto& handler) {
...
void AgoraRteScene::OnRemoteStreamStats(const std::string& stream_id, const RemoteStreamStats& stats) {
  AgoraRteUtils::NotifyEventHandlers<IAgoraRteSceneEventHandler>(
    callback_lock_, event_handlers_,
    [&](const auto& handler) {
...
inline int AgoraRteScene::UnpublishMediaPlayer(
    std::shared_ptr<IAgoraRteMediaPlayer> player) {
  int result1 = -ERR_NOT_READY, result2 = -ERR_NOT_READY;
  auto video_track =
      std::static_pointer_cast<AgoraRteMediaPlayer>(player)->GetVideoTrack();
  if (video_track) {
...
#if defined(RTE_LINUX)
  // It must be set as false in linux and win32 platform, otherwise it'will
  // initialize fialure
  svc_cfg_ex.enableAudioDevice = false;
#else
...
// Notes for token format V1, take {"v":"1","u":"user1","s":"test"} as example,
// 1. Begin with {"v":"
// 2. Then version number which can't be empty
// 3. Then ","u":"
// 4. Then user id which can't be empty
// 5. Then ","s":"
// 6. Then stream id which could be empty for major stream, add '?' for
// no-greedy matching in case there is extension behind
// 7. Then any others extension which we don't care for now
// 8. End with }
//
// Do _not_ add whitespace between json elements.
// Agora requires user name or stream name shouldn't contains "
//
static const std::string g_rte_user_regex =
    "^\\{\"v\":\"([0-9]+)\",\"u\":\"(.+)\",\"s\":\"(.*?)\".*\\}$";
static const std::string g_rte_token_format = R"({"v":"%s","u":"%s","s":"%s"})";
...
inline std::string AgoraRteUtils::GenerateRtcStreamId(
  ...
  if (is_major_stream) {
    ...
    result =
        GeneratorJsonUserId(g_rte_token_format, g_rte_token_version.c_str(),
                            rtc_user_id.c_str(), empty.c_str());
  } else {
    result =
        GeneratorJsonUserId(g_rte_token_format, g_rte_token_version.c_str(),
...
inline bool AgoraRteUtils::ExtractRtcStreamInfo(
  ...
  // A example of rtc major connection, for 'user1' user, the id is
  // {"v":"1","u":"user1","s":""}
  // A example of rtc normal connection for 'user1' user and 'test' stream , the
  // {"v":"1","u":"user1","s":"test"}

  std::smatch sm;

  if (std::regex_search(rtc_stream_id, sm, std::regex(g_rte_user_regex))) {
    std::string version = sm[1];
    info.user_id = sm[2];
    info.stream_id = sm[3];
    info.is_major_stream = info.stream_id.empty();
    return true;
  }
  return false;
}
...
AgoraRteVideoDeviceManager::EnumerateVideoDevices() {
  ...
  if (device_info) {
    ...
    for (int i = 0; i < number; i++) {
      ...
      char id[max_string_len] = {0};
      if (device_info->GetDeviceName(i, device_name, max_string_len, id,
                                     max_string_len, device_id,
