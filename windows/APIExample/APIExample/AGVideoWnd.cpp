...
void CAGInfoWnd::SetLocalStats(const LocalStats stat)
{
	localstat = stat;
	if (m_bShowTip) {
		Invalidate(TRUE);
...
void CAGInfoWnd::SetRemoteStats(const RemoteStats stat)
{
	remotestat = stat;
	if (m_bShowTip) {
		Invalidate(TRUE);
...
void CAGInfoWnd::OnPaint()
{
	...
	if (m_bShowTip) {
		...
		strTip = str;
		if (m_bLocal) {
			str.Format(_T("%dx%d,enc:%dfps,render:%dfps\n")
				, localstat.stats.video_stats.encodedFrameWidth, localstat.stats.video_stats.encodedFrameHeight
				, localstat.stats.video_stats.encoderOutputFrameRate
				, localstat.stats.video_stats.rendererOutputFrameRate);
			strTip += str;

			str.Format(_T("%dhz %dch tx:%dkbps\n")
				, localstat.stats.audio_stats.sentSampleRate, localstat.stats.audio_stats.numChannels
				, localstat.stats.audio_stats.sentBitrate);

			strTip += str;
		}
		else {
			str.Format(_T("%dx%d,dec:%dfps,render:%dfps\n")
				, remotestat.stats.video_stats.width, remotestat.stats.video_stats.height
				, remotestat.stats.video_stats.decoderOutputFrameRate
				, remotestat.stats.video_stats.rendererOutputFrameRate);
			strTip += str;

			str.Format(_T("rx:%dkbps,frameLoss:%d\n delay:%d,packetLoss:%d\n")
				, remotestat.stats.video_stats.receivedBitrate, remotestat.stats.video_stats.frameLossRate
				, remotestat.stats.video_stats.delay, remotestat.stats.video_stats.packetLossRate);
			strTip += str;

			str.Format(_T("audio:%dhz %dch tx:%dkbps\n")
				, remotestat.stats.audio_stats.receivedSampleRate, remotestat.stats.audio_stats.numChannels
				, remotestat.stats.audio_stats.receivedBitrate);

			strTip += str;

			str.Format(_T("network:%dms, jitter%dms, loss:%d\n")
				, remotestat.stats.audio_stats.networkTransportDelay, remotestat.stats.audio_stats.jitterBufferDelay
				, remotestat.stats.audio_stats.audioLossRate);
...
void CAGVideoWnd::SetUID(std::string nUID)
{
	m_nUID = nUID;
...
void CAGVideoWnd::SetStrmId(std::string strmId)
{
	m_strmId = strmId;
	
	m_wndInfo.SetStrmId(strmId);
	//if (strmId.empty())
	//	m_wndInfo.ShowWindow(SW_HIDE);
	//else
	//	m_wndInfo.ShowWindow(SW_SHOW);
...
void CAGVideoWnd::SetLocalFlag(BOOL bLocal)
{
	CRect	rcClient;

	m_wndInfo.m_bLocal = bLocal;
	GetClientRect(&rcClient);

	int width = WND_INFO_WIDTH;
	int height = WND_INFO_HEIGHT;
	if (!m_wndInfo.m_bLocal) {
		width = WND_INFO_WIDTH * 3 / 2;
		height = WND_INFO_HEIGHT * 3 / 2;
	}

	int x = (rcClient.Width()- width) / 2;
	int y = rcClient.Height() - height;
	
	if (m_wndInfo.GetSafeHwnd() != NULL) {
		m_wndInfo.MoveWindow(x, y, width, height);
	}
};
