...
class IAgoraRteScene {
 public:
  ...
  virtual int CreateOrUpdateRTCStream(const std::string& local_stream_id,
                                      const RtcStreamOptions& option) = 0;
  ...
  virtual int CreateOrUpdateDirectCDNStream(
      ...
      const DirectCdnStreamOptions& option) = 0;
   ...
   * This method will trigger 'OnBypassCdnStateChanged' and
   'OnBypassCdnPublished' callback
   *
   * @param local_stream_id ID of the local stream to be destroyed
   ...
   *        If you set this parameter as `true`, ensure that you call the
   `SetBypassCdnTranscoding`
   * method before this method.
   * @return
   * - 0: Success.
   * - < 0: Failure.
   */
  virtual int AddBypassCdnUrl(const std::string& local_stream_id,
                              const std::string& target_cdn_url,
                              bool transcoding_enabled) = 0;

  /**
   * Remove a bypass CDN URL from a local stream
   * This method will trigger 'OnBypassCdnStateChanged' and
   'OnBypassCdnUnpublished' callback
   *
   * @param local_stream_id ID of the local stream to be destroyed
   * @param target_cdn_url The CDN streaming URL in the RTMP format.
             The maximum length of this parameter is 1024 bytes.
   ...
   * - < 0: Failure.
   */
  virtual int RemoveBypassCdnUrl(const std::string& local_stream_id,
  ...
  virtual void RegisterAudioFrameObserver(
      ...
      const AudioObserverOptions& option) = 0;
...
class IAgoraRteSceneEventHandler {
 public:
   ...
   */
  virtual void OnLocalStreamStateChanged(const StreamInfo& streams,
   ...
   * @param reason The reason of the state change.
   */
  virtual void OnRemoteStreamStateChanged(const StreamInfo& streams,
                                          MediaType media_type,
                                          StreamMediaState old_state,
                                          StreamMediaState new_state,
                                          StreamStateChangedReason reason) = 0;
   ...
   * @param totalVolume Total volume after audio mixing. The value ranges
   * between 0 (lowest volume) and 255 (highest volume).
   */
  virtual void OnAudioVolumeIndication(
      const std::vector<AudioVolumeInfo>& speakers, int totalVolume) = 0;
   ...
   */
  virtual void OnSceneTokenWillExpired(const std::string& scene_id,
   ...
   * the user will drop offline. This callback is triggered 30 seconds before
  ...
  virtual void OnBypassCdnStateChanged(
      ...
      CDNBYPASS_STREAM_PUBLISH_ERROR err_code) = 0;
  ...
  virtual void OnBypassTranscodingUpdated(const std::string& stream_id) = 0;

  virtual void OnSceneStats(const SceneStats& stats) = 0;

  virtual void OnLocalStreamStats(const std::string& stream_id,const LocalStreamStats& stats) = 0;

  virtual void OnRemoteStreamStats(const std::string& stream_id, const RemoteStreamStats& stats) = 0;
